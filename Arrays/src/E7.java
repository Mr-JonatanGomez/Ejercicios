...
Una vez hecho esto saldrá un menú que me dará las siguientes opciones:
    ...
    x(con ayudaGPT)- Ver palabra con más letras
    x(con ayudaGPT)- Ver palabra con menos letras
    */
public class E7 {
    public static void main(String[] args) {
        ...
        String palabraMasLarga = palabras[0];// comparamos con una posicion
        for (String item : palabras) { // buscamos en palabras los item
            if (item.length() > palabraMasLarga.length()) { //si long.item() mas que palabraLarga.length()
                palabraMasLarga = item;
            }
        }
        System.out.println("Esta es la palabra con más letras: " + palabraMasLarga);
